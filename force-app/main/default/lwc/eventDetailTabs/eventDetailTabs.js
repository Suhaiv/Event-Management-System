...
export default class EventDetailTabs extends NavigationMixin(LightningElement) {
  ...
  wiredEvent({ data, error }) {
    if (data) {
      ...
      this.eventStatus = data.fields.Status__c?.value;

      this.eventStart = data.fields.Start__c?.value;

      this.eventEnd = data.fields.End__c?.value;

      this.maxSeats = data.fields.Max_Seats__c?.value;
    ...
    getSpeakers({
      ...
      .then((result) => {
        this.speakers = result.map((record) => ({
          ...
          name: record.Speaker__r ? record.Speaker__r.Name : "",

          email: record.Speaker__r ? record.Speaker__r.Email__c : "",

          phone: record.Speaker__r ? record.Speaker__r.Phone__c : "",

          company: record.Speaker__r ? record.Speaker__r.Company__c : ""
        }));
      })
      .catch((error) => {
    ...
    getAttendees({
      ...
      .then((result) => {
        this.attendees = result.map((record) => ({
          ...
          name: record.Attendees__r ? record.Attendees__r.Name : "",

          email: record.Attendees__r ? record.Attendees__r.Email__c : "",

          phone: record.Attendees__r ? record.Attendees__r.Phone__c : "",
